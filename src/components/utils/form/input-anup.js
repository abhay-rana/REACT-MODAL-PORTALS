...
const Input = memo(
	forwardRef((props, ref) => {
		...
		const { className, error, success, small, large, children, label, stacked, no_gap, light, note, name } = { ...props };

		let extraClass =
			"w-full z-0 box-border text-gray-dark focus:border-primary focus:outline-none placeholder-gray-light group-hover:border-primary focus:border-2";
		if (small) {
		...
		return (
			<div className="group relative">
				...
				{label ? (
					<label
						htmlFor={name}
						className={`absolute -top-4 mb-1 block text-xs font-normal ${
